        ...
        >
          ...
          <div style={{
            ...
            <GlassPopup isOpen={showOrgPopup} onClose={() => setShowOrgPopup(false)}>
              ...
              {/* Actions */}
              <PopupItem index={0} icon={UserCog}     label="Gérer les membres"   href="/settings/members" />
              <PopupItem index={1} icon={Blocks}      label="Intégrations"         href="/settings/integrations" />

              <PopupDivider />

              <PopupItem index={2} icon={Plus}        label="Créer une organisation" href="/select-org" />
              <PopupItem index={3} icon={Building2}   label="Rejoindre une organisation" href="/select-org" />
            ...
            <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
              <ScrollArea style={{ flex: 1, padding: "var(--space-2)" }}>
                <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-1)", width: "100%" }}>
                  ...
                  <NavItem href="/dashboard"        icon={LayoutDashboard}  label="Tableau de bord"      isCollapsed={isCollapsed} isActive={!!pathname?.includes("dashboard")} />
                  <NavItem href="/reports"           icon={FileClock}        label="Rapports"             isCollapsed={isCollapsed} isActive={!!pathname?.includes("reports")} />
                  <NavItem href="/chat"              icon={MessagesSquare}   label="Chat"                 isCollapsed={isCollapsed} isActive={!!pathname?.includes("chat")}     badge={betaBadge} />

                  <SidebarDivider />

                  <NavItem href="/deals"             icon={Layout}           label="Deals"                isCollapsed={isCollapsed} isActive={!!pathname?.includes("deals")} />
                  <NavItem href="/accounts"          icon={UserCircle}       label="Comptes"              isCollapsed={isCollapsed} isActive={!!pathname?.includes("accounts")} />
                  <NavItem href="/competitors"       icon={UserSearch}       label="Concurrents"          isCollapsed={isCollapsed} isActive={!!pathname?.includes("competitors")} />

                  <SidebarDivider />

                  <NavItem href="/library/knowledge" icon={GraduationCap}   label="Base de connaissance" isCollapsed={isCollapsed} isActive={!!pathname?.includes("library")} />
                  <NavItem href="/feedback"          icon={MessageSquareText}label="Feedback"             isCollapsed={isCollapsed} isActive={!!pathname?.includes("feedback")} />
                  <NavItem href="/review"            icon={FileClock}        label="Revue documentaire"   isCollapsed={isCollapsed} isActive={!!pathname?.includes("review")} />
