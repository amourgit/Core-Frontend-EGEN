...
import { SESSION_MONITOR, INACTIVITY, } from '../security/constants';
// Grace period post-login (session Keycloak pas encore propagée)
const LOGIN_GRACE_PERIOD_MS = 8000;
// ── useSessionMonitor ─────────────────────────────────────────
export function useSessionMonitor() {
    ...
    const checkSession = useCallback(async () => {
        ...
        if (timeSinceAuth < LOGIN_GRACE_PERIOD_MS)
            return;
...
// ── useInactivityGuard ────────────────────────────────────────
export function useInactivityGuard(onWarning) {
    const { isAuthenticated, logout, user, sessionId } = useAuthContext();
    ...
    const resetTimers = useCallback(() => {
        ...
        timerRef.current = setTimeout(async () => {
            auditLogger.logInactivityTimeout(window.location.pathname, user?.id, sessionId || undefined);
            tokenManager.clear();
            await logout(true);
    ...
    useEffect(() => {
        ...
        INACTIVITY.RESET_EVENTS.forEach(evt => window.addEventListener(evt, handleActivity, { passive: true }));
        return () => {
            clearTimers();
            INACTIVITY.RESET_EVENTS.forEach(evt => window.removeEventListener(evt, handleActivity));
        };
