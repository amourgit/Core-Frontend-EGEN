 ...
 */
function isKnownRootDomain(host, extraRoots = []) {
    const allRoots = [...ROOT_DOMAINS, ...extraRoots];
    return allRoots.some((root) => host === root || host === `www.${root}`);
}
/**
 * Détecte la longueur du TLD composé si applicable.
 * Exemple : "co.uk" → 2, "com" → 1
 */
function getCompoundTLDLength(parts) {
    if (parts.length < 2)
        return 1;
 ...
 */
function extractAllSubdomains(hostname, options = {}) {
 ...
 * // r.requested = ["tenant"]
 */
export function resolveSubdomains(options = {}) {
    const { hostname: explicitHostname, levels, additionalRootDomains = [], ignoreWww = true, } = options;
    // Déterminer le hostname source
    const hostname = explicitHostname ??
 ...
 */
export function getSubdomainAt(level, hostname) {
 ...
 */
export function getSubdomainsList(levels, hostname) {
 ...
 * getTopSubdomain() // → "myclient"
 */
export function getTopSubdomain(hostname) {
 ...
 */
export function getAllSubdomainsMap(hostname) {
 ...
 * }
 */
export function useSubdomains(options = {}) {
